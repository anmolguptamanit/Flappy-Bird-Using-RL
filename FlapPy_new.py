...
class QNetwork():
    ...
    def build_model(self, action_size, scope):
        with tf.variable_scope(scope):
            hidden1 = tf.layers.dense(self.state_in, 300, activation=tf.nn.relu)
            hidden2 = tf.layers.dense(hidden1, 200, activation=tf.nn.relu)
            q_state = tf.layers.dense(hidden2, action_size, activation=None)
            return q_state
...
class DoubleDQNAgent():
    def __init__(self, state_dim, action_size):
        ...
        self.replay_buffer = ReplayBuffer(maxlen=40000)
        self.gamma = 0.97
        self.eps = 1.0
    ...
    def get_action(self, state):
        ...
        action_random = 0 #np.random.randint(self.action_size)
    ...
    def train(self, state, action, next_state, reward, done, use_DDQN=True):
        ...
        if done: self.eps = max(0.1, 0.999 * self.eps)
...
def mainGame(movementInfo, agent):
    ...
    reward = 0
    state_prev = [playery, upperPipes[0]['x'], upperPipes[0]['y'], lowerPipes[0]['x'], lowerPipes[0]['y'], playerVelY]
    ...
    while True:
        ...
        if len(upperPipes) > 0 and upperPipes[0]['x'] < -IMAGES['pipe'][0].get_width() - 15:
            upperPipes.pop(0)
            lowerPipes.pop(0)
        ...
        if counter ==0:
            state_param_1 = upperPipes[0]['x']
            state_param_2 = upperPipes[0]['y']
            state_param_3 = lowerPipes[0]['x']
            state_param_4 = lowerPipes[0]['y']
        else:
            state_param_1 = upperPipes[1]['x']
            state_param_2 = upperPipes[1]['y']
            state_param_3 = lowerPipes[1]['x']
            state_param_4 = lowerPipes[1]['y']


        crashTest = checkCrash({'x': playerx, 'y': playery, 'index': playerIndex},
                               upperPipes, lowerPipes)
        if crashTest[0]:
            state_next = [playery, state_param_1, state_param_2, state_param_3, state_param_4, playerVelY]
            agent.train(state_prev, action, state_next, -1000, done = 1, use_DDQN=True)
        ...
        agent.train(state_prev, action, state_next, reward_loop, done = 0, use_DDQN=True)
        state_prev = state_next

        #reward_epsiode += reward_loop
